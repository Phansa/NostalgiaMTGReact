...
if __name__ == "__main__":
    ...
    with open("../data/AllSets.json", encoding="utf8") as json_file:
        ...
        for set_object in data:
            ...
            if(set_name == "CON"):
                set_name = "CON_"
            if(not(os.path.isdir(folderPath + set_name))):
                os.makedirs(folderPath + set_name)
            for card_name in data[set_object]['cards']:
                ...
                if(not(os.path.isfile(folderPath + set_name + "/" + str(card_name['imageName']) + ".jpg"))):
                    image_link = ur.urlopen("http://gatherer.wizards.com/Handlers" +
                    "/Image.ashx?multiverseid=" + 
                    str(card_name['multiverseid']) + "&type=card")
                    out_path = open(folderPath + set_name + "/" + str(card_name['imageName']) + ".jpg",
                                        "wb")
